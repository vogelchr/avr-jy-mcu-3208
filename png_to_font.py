...
print('Image %s has size %d,%d.'%(args[1],IMG.size[0],IMG.size[1]), file=sys.stderr)
...
print('\t'+',\n\t'.join(['0x%02x /* %s */'%(v,bitpattern(v)) for v in pixel_data_arr]))
print('};')

print('static const PROGMEM struct font_offset %s_offs[]={'%(symbol_name))
print('\t'+',\n\t'.join(['{ .code=0x%02x, .offs=0x%04x }'%pair for pair in character_offset_arr]))
print('};')
