...
def parse_char_range(s) :
	...
	ret = list()
	for elm in s.split(',') :
		arr2 = map(char_or_number_to_int,elm.split('-'))
		if len(arr2) == 1 :
			ret.append(arr2[0])
			continue
		if len(arr2) == 2 :
			ret = ret+range(arr2[0],arr2[1]+1)
			continue
...
print >>sys.stderr,'Image %s has size %d,%d.'%(args[1],IMG.size[0],IMG.size[1])
...
print '\t'+',\n\t'.join(map(lambda v:'0x%02x'%(v),pixel_data_arr))
print '};'

print 'static const PROGMEM struct font_offset %s_offs[]={'%(symbol_name)
print '\t'+',\n\t'.join(map(lambda pair:'{ .code=0x%02x, .offs=0x%04x }'%pair,character_offset_arr))
print '};'
